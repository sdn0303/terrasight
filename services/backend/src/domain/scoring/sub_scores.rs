...
///
/// # Parameters
/// - `nearest_dist_m`: Distance to nearest station (metres)
/// - `best_ridership`: Highest daily ridership among stations within range
/// - `line_count_1km`: Number of distinct rail lines within 1km
pub fn score_transit(
    nearest_dist_m: Option<f64>,
    best_ridership: Option<i64>,
    line_count_1km: Option<i64>,
) -> f64 {
    match (nearest_dist_m, best_ridership, line_count_1km) {
        (Some(dist), Some(ridership), Some(lines)) => {
            let distance_factor = (TRANSIT_DISTANCE_MAX - dist / TRANSIT_DISTANCE_DIVISOR).max(0.0);
            let ridership_factor = if ridership > 0 {
                ((ridership as f64).log10() * TRANSIT_RIDERSHIP_LOG_MULTIPLIER).min(SCORE_MAX)
            } else {
                0.0
            };
            let multi_line_bonus = ((lines - 1).max(0) as f64) * TRANSIT_MULTI_LINE_BONUS;

            (distance_factor * TRANSIT_WEIGHT_DISTANCE
                + ridership_factor * TRANSIT_WEIGHT_RIDERSHIP
                + multi_line_bonus * TRANSIT_WEIGHT_MULTI_LINE)
                .min(SCORE_MAX)
        }
        _ => UNAVAILABLE_DEFAULT,
    }
}
...
/// Population dynamics score from 2020→2050 change rate. `None` = unavailable = 100.
pub fn score_population(change_rate: Option<f64>) -> f64 {
    match change_rate {
        None => UNAVAILABLE_DEFAULT,
        Some(rate) => {
            for &(lower, score) in POPULATION_MAP {
                if rate >= lower {
                    return score;
                }
            }
...
mod tests {
    ...
    #[test]
    fn transit_unavailable() {
        assert_eq!(score_transit(None, None, None), 100.0);
    }

    #[test]
    fn transit_close_high_ridership() {
        // 200m to station, 100k ridership, 3 lines
        let s = score_transit(Some(200.0), Some(100_000), Some(3));
        assert!(s > 70.0, "close + high ridership should score high: {s}");
    }

    #[test]
    fn transit_far_low_ridership() {
        let s = score_transit(Some(1500.0), Some(1_000), Some(1));
        assert!(s < 40.0, "far + low ridership should score low: {s}");
    }
    ...
    #[test]
    fn population_unavailable() {
        assert_eq!(score_population(None), 100.0);
    }

    #[test]
    fn population_thresholds() {
        assert_eq!(score_population(Some(0.30)), 100.0);
        assert_eq!(score_population(Some(0.20)), 100.0);
        assert_eq!(score_population(Some(0.15)), 85.0);
        assert_eq!(score_population(Some(0.05)), 65.0);
        assert_eq!(score_population(Some(-0.05)), 40.0);
        assert_eq!(score_population(Some(-0.15)), 20.0);
        assert_eq!(score_population(Some(-0.25)), 5.0);
    }
