...
- admin-boundary FGBのN03_*をprefName/cityName等にリネーム。クリック→selectArea()配線完了。boundary-layerラベル修正

### ~~X-05: popupFields / schema / asset drift → LayerRegistry 統合~~ ✅
- 11個のpopupFieldキー不一致を修正（price→price_per_sqm等）。バリデーションテスト追加で再発防止。136テスト通過

### ~~X-06: bbox 近似 → map.getBounds() 化~~ ✅
- map.getBounds()をonMoveEndコールバック経由で渡す方式に変更。handleLoadで初回bbox発行。近似はfallbackのみ。テスト122本通過
...
- 337行→~70行。useMapPage/useMapInteraction/LayerRendererに分割。AnalysisStoreをMapStoreに統合。8コミット完了

### ~~モードシステム簡略化（3→2モード）~~ ✅
- explore/analyze/compare → explore/compare。自動モード切替廃止。プログレッシブ・ディスクロージャーでexplore内に統合

### ~~エラー状態の可視化~~ ✅
- StatusBarにWASMエラー・エリアデータエラー・ズーム警告を表示。マップにズームオーバーレイ追加。テスト128本通過

### ~~ガイド付き初回体験~~ ✅
- 初回訪問時に安全性テーマを自動アクティブ化。テーマ状態をURLパラメータに永続化。再訪問時は保存状態を復元

### ~~空間ポップアップ（MapLibre Popup連携）~~ ✅
- react-map-gl Popupでクリック座標にアンカー。ダークテーマCSS適用。画面中央固定を廃止

### ~~レイヤーキュレーション（テーマカード大型化）~~ ✅
- 2×2小ボタン→縦型カードにリデザイン。アイコン・説明文・レイヤー数を表示。i18n対応。134テスト通過
...
### ~~デザインシステム実装~~ ✅
- @theme inlineにds-*トークンをマッピング。18ファイル60+箇所のハードコードTailwindクラスをトークンに置換。132テスト通過
