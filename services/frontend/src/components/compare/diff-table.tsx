      ...
      <table className="w-full text-[10px]">
        <thead>
          <tr className="text-neutral-500">
            ...
            <th className="text-right py-1 font-normal text-cyan-400">A</th>
        ...
        <tbody>
          {rows.map(({ key, label, a, b }) => {
            const delta = a - b;
            return (
              <tr key={key} className="text-neutral-400">
                <td className="py-0.5">{label}</td>
                <td className="text-right font-mono">{a}</td>
                <td className="text-right font-mono">{b}</td>
                <td className={`text-right font-mono ${delta > 0 ? "text-cyan-400" : delta < 0 ? "text-amber-400" : "text-neutral-600"}`}>
          ...
          <tr className="border-t border-neutral-700 text-neutral-200">
            ...
            <td className="text-right font-mono font-medium">{Math.round(tlsA)}</td>
            <td className="text-right font-mono font-medium">{Math.round(tlsB)}</td>
            <td className={`text-right font-mono font-medium ${tlsA > tlsB ? "text-cyan-400" : tlsB > tlsA ? "text-amber-400" : "text-neutral-600"}`}>
